...
| Kapsam dışı | `~/.claude/plugins/` cache dizini (yüzlerce MB) ve konuşma transcript'leri | Plugin cache'i yerine `enabledPlugins`/`extraKnownMarketplaces` taşınır. **2026-08-17 güncellemesi:** bu iki anahtarın tek başına hiçbir şey kurmadığı doğrulandı (Claude Code, `enabledPlugins`'i yalnızca "zaten kuruluysa aç/kapa" olarak okuyor, eksikse indirmiyor). Bunun üzerine `src/plugins-sync.mjs` eklendi: `pull` elle (hook'suz, TTY'li) çalıştırıldığında, synced `enabledPlugins`'te olup bu makinede kurulu olmayan her plugin için tek tek "kurulsun mu?" diye soruyor, evet denirse `claude plugin install` çalıştırıyor, hayır denirse `~/.agent-sync/plugin-choices.json`'a kaydedip bir daha sormuyor. Hook'tan ve VS Code eklentisinden asla çalışmaz (ikisinde de soru sorulacak terminal yok); transcript'ler hâlâ Faz 2, `syncTranscripts: false` varsayılanıyla kapalı |
