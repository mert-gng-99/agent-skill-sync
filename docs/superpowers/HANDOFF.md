...
| 3 | Senkron karar motoru (saf) | `src/state.mjs`, `src/sync-engine.mjs` | ✅ **Tamamlandı** |
...
| Kapsam dışı | `~/.claude/plugins/` cache dizini (yüzlerce MB) ve konuşma transcript'leri | Plugin cache'i yerine `enabledPlugins`/`extraKnownMarketplaces` taşınır. **2026-08-17 güncellemesi:** bu iki anahtarın tek başına hiçbir şey kurmadığı doğrulandı (Claude Code, `enabledPlugins`'i yalnızca "zaten kuruluysa aç/kapa" olarak okuyor, eksikse indirmiyor). Bunun üzerine `src/plugins-sync.mjs` eklendi: `pull` elle (hook'suz, TTY'li) çalıştırıldığında, synced `enabledPlugins`'te olup bu makinede kurulu olmayan her plugin için tek tek "kurulsun mu?" diye soruyor, evet denirse `claude plugin install` çalıştırıyor, hayır denirse `~/.agent-sync/plugin-choices.json`'a kaydedip bir daha sormuyor. Hook'tan ve VS Code eklentisinden asla çalışmaz (ikisinde de soru sorulacak terminal yok). **2026-08-18 güncellemesi:** `syncTranscripts` varsayılanı kullanıcının açık isteğiyle `false`'tan `true`'ya çevrildi (`src/config.mjs`, `DEFAULT_CONFIG`) - artık `init` ile kurulan her yeni makinede baştan açık geliyor. Zaten var olan bir `config.json`'da yazılı `syncTranscripts: false` bu değişiklikle kendiliğinden `true` olmaz, o makinede elle değiştirilmesi gerekir. Ev dizini gibi proje sayılmayan yerlerde başlayan oturumlar bu ayardan bağımsız olarak hâlâ hiç senkronlanmıyor (bkz. bölüm 6'daki proje kısıtı) |
