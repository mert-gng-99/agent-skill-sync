...
- `agent-sync: Add this project to Brain` bu klasörün kendi `.claude/settings.local.json`'ını, `vaultPath`'te ayarlı avenoxbeyin (veya benzeri) vault'a bağlar (bkz. bölüm 3), böylece bu projenin oturumları da tıpkı vault'un kendi içindeki oturumlar gibi o vault'un `daily/` logu'na akmaya başlar. Önce bir vault bağlanmış olması gerekir (`agent-sync vault`); yoksa hiçbir şey yapmaz.
